...
def compute_summary_stats(times: Union[np.ndarray, list], 
                         charges: Union[np.ndarray, list]) -> Dict[str, float]:
    """
    ...
    Returns:
        Dictionary containing the 9 summary statistics:
        - total_charge: Total charge collected by the sensor
        - charge_100ns: Charge within 100ns of first pulse
        - charge_500ns: Charge within 500ns of first pulse
        - first_pulse_time: Time of the first pulse
        - last_pulse_time: Time of the last pulse
        ...
        - charge_weighted_mean_time: Charge-weighted mean of pulse times
        - charge_weighted_std_time: Charge-weighted standard deviation of pulse times
    ...
    return {
        ...
        'charge_100ns': float(charge_100ns),
        'charge_500ns': float(charge_500ns),
        'first_pulse_time': float(first_pulse_time),
        'last_pulse_time': float(last_pulse_time),
        'charge_20_percent_time': float(charge_20_percent_time),
        'charge_50_percent_time': float(charge_50_percent_time),
        'charge_weighted_mean_time': float(charge_weighted_mean_time),
        'charge_weighted_std_time': float(charge_weighted_std_time)
    }


def _empty_stats() -> Dict[str, float]:
    """Return empty statistics dictionary for zero-length inputs."""
    return {
        'total_charge': 0.0,
        'charge_100ns': 0.0,
        'charge_500ns': 0.0,
        ...
        'last_pulse_time': 0.0,
        'charge_20_percent_time': 0.0,
        'charge_50_percent_time': 0.0,
        'charge_weighted_mean_time': 0.0,
        'charge_weighted_std_time': 0.0
    }


def compute_summary_stats_batch(times_list: list, charges_list: list) -> list:
    """
    ...
    Returns:
        List of summary statistics dictionaries, one per sensor
    """
