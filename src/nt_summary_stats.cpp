...
std::pair<std::vector<double>, std::vector<double>> group_hits_by_window(
    ...
    auto current_bin = static_cast<long long>(std::floor((times.front() - base_time) / window_ns));
    double bin_charge = charges.front();
    double bin_time = times.front();

    for (std::size_t i = 1; i < times.size(); ++i) {
        const double time = times[i];
        const auto bin = static_cast<long long>(std::floor((time - base_time) / window_ns));
        if (bin == current_bin) {
...
std::array<double, kNumStats> compute_stats_from_sorted(
    ...
    const double total_charge = std::accumulate(charges.begin(), charges.end(), 0.0);
    if (n == 1) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        const double time = times[i];
        sum_qt += charge * time;
        sum_qt2 += charge * time * time;
    }

    const auto it_100 = std::upper_bound(times.begin(), times.end(), first_time + 100.0);
    const auto it_500 = std::upper_bound(times.begin(), times.end(), first_time + 500.0);
    const std::size_t idx_100 = static_cast<std::size_t>(it_100 - times.begin());
    const std::size_t idx_500 = static_cast<std::size_t>(it_500 - times.begin());

    const double charge_100 = idx_100 > 0 ? cumulative[idx_100 - 1] : 0.0;
    const double charge_500 = idx_500 > 0 ? cumulative[idx_500 - 1] : 0.0;

    const double threshold_20 = total_charge * 0.2;
    const double threshold_50 = total_charge * 0.5;
    const auto it_20 = std::lower_bound(cumulative.begin(), cumulative.end(), threshold_20);
    const auto it_50 = std::lower_bound(cumulative.begin(), cumulative.end(), threshold_50);
    const std::size_t idx_20 = static_cast<std::size_t>(std::min<std::ptrdiff_t>(it_20 - cumulative.begin(), static_cast<std::ptrdiff_t>(n - 1)));
    const std::size_t idx_50 = static_cast<std::size_t>(std::min<std::ptrdiff_t>(it_50 - cumulative.begin(), static_cast<std::ptrdiff_t>(n - 1)));
    const double charge_20_time = times[idx_20];
    const double charge_50_time = times[idx_50];
...
py::array_t<double> compute_summary_stats_py(py::array_t<double, py::array::c_style | py::array::forcecast> times,
    ...
    auto stats = compute_stats_single_sensor_impl(std::move(times_vec), std::move(charges_vec), std::nullopt);
...
py::array_t<double> process_sensor_data_py(
    ...
    auto stats = compute_stats_single_sensor_impl(std::move(times_vec), std::move(charges_vec), grouping_window_ns);
    return to_array(stats);
}
...
py::tuple process_event_arrays_py(
    ...
    std::vector<int32_t> sorted_string_ids(n_hits);
    std::vector<int32_t> sorted_sensor_ids(n_hits);
    std::vector<double> sorted_times(n_hits);
    std::vector<double> sorted_charges(n_hits);
    std::vector<double> sorted_pos_x(n_hits);
    std::vector<double> sorted_pos_y(n_hits);
    std::vector<double> sorted_pos_z(n_hits);
    ...
    for (std::size_t i = 0; i < n_hits; ++i) {
        ...
        sorted_string_ids[i] = string_ptr[idx];
        sorted_sensor_ids[i] = sensor_ptr[idx];
        sorted_times[i] = times_ptr[idx];
        sorted_charges[i] = charges_vec[idx];
        sorted_pos_x[i] = pos_x_ptr[idx];
        sorted_pos_y[i] = pos_y_ptr[idx];
        sorted_pos_z[i] = pos_z_ptr[idx];
    }
    ...
    std::vector<std::array<double, 3>> sensor_positions;
    sensor_positions.reserve(n_hits);

    for (std::size_t i = 0; i < n_hits; ++i) {
        if (i == 0 ||
            sorted_string_ids[i] != sorted_string_ids[i - 1] ||
            sorted_sensor_ids[i] != sorted_sensor_ids[i - 1]) {
            if (i != 0) {
                sensor_offsets.push_back(i);
            }
            sensor_positions.push_back(
                {sorted_pos_x[i], sorted_pos_y[i], sorted_pos_z[i]}
            );
        }
    }
    sensor_offsets.push_back(n_hits);

    const std::size_t n_sensors = sensor_positions.size();
    ...
    for (std::size_t i = 0; i < n_sensors; ++i) {
        positions_buf(i, 0) = sensor_positions[i][0];
        positions_buf(i, 1) = sensor_positions[i][1];
        positions_buf(i, 2) = sensor_positions[i][2];
    }

    auto stats_buf = stats.mutable_unchecked<2>();
    for (std::size_t i = 0; i < n_sensors; ++i) {
        const std::size_t start = sensor_offsets[i];
        const std::size_t end = sensor_offsets[i + 1];
        std::vector<double> times_slice(sorted_times.begin() + start, sorted_times.begin() + end);
        std::vector<double> charges_slice(sorted_charges.begin() + start, sorted_charges.begin() + end);
        auto sensor_stats = compute_stats_single_sensor_impl(
            std::move(times_slice),
            std::move(charges_slice),
            grouping_window_ns
        );
        for (std::size_t j = 0; j < kNumStats; ++j) {
            stats_buf(i, j) = sensor_stats[j];
        }
    }
